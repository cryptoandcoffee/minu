...
#if defined(NDEBUG)
# error "Bitocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2f364b26fffc0b663b7d9a22ec6411e552e1b4e27a36149b0ac2c03bfb809a25");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitocoin Signed Message:\n";
...
{
    // Bitocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 2)
    {
        nSubsidy =  10000000 * COIN; //10 million premine
    }
    else if(nHeight < 262500) //Disabled  
    {
        nSubsidy = 200 * COIN; //Double mining in first 3 months
    }
    // Subsidy is cut in half every 4200000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 4200000); // Bitocoin: 4200000 blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Bitocoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Bitocoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2f364b26fffc0b663b7d9a22ec6411e552e1b4e27a36149b0ac2c03bfb809a25");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "New York Times 04/Apr/2021 Vaccinations Rise, but Variants and Factory Mix-Up Present Hurdles";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047384710fa689ad5123690c33f3a49c3f13f8d45b7c157fbcbc8bc4a8e4d3eb4b10f8d4601fa08dce999aaf0f470216fe1b11850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1617551661;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1177208;
        ...
        assert(block.hashMerkleRoot == uint256("0x9dabf9379240f4eaf2e96e3e0fe14501cbfb29115ddaba490abce9cfc4b24841"));
...
unsigned char pchMessageStart[4] = { 0xcf, 0xdf, 0xef, 0xaf }; // Bitocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitocoinMiner(CWallet *pwallet)
{
    printf("BitocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitocoin-miner");
    ...
    try { loop {
        ...
        printf("Running BitocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitocoinMiner, pwallet));
}
