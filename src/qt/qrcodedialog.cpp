...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BTO")
    updateDisplayUnit();
}
...
{
    QString ret = QString("bitocoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BTO unit input in lnReqAmount, we generate the URI with BTO as unit (as defined in BIP21)
